function parse(nextToken){
	...
	function readStatement(){
		...
		switch(type){
			...
			break;case "CICLOP":
				current.type="CICLOP";
				noEquals=true;
				assert(current.variable=readExpression(),"desaparecida CICLOP condicion");
				noEquals=false;
				console.log(type,word)
				assert(readToken("="),"desaparecida = en CICLOP");
				noTo=true;
				current.start=readExpression();
				noTo=false;
				assert(readToken("LIMITE"),"desaparecida TO en CICLOP");
				...
				startBlock();
			break;case "FNCICLOP":
				assert(currentBlock().type=="CICLOP","FNCICLOP sin CICLOP");
				readExpression();
				endBlock();
			
			break;case "CICLOMQ":
				current.type="CICLOMQ";
				assert(current.condition=readExpression(),"Falta el argumento de la palabra clave");
				startBlock();
			break;case "FNCICLOMQ":
				assert(currentBlock().type=="CICLOMQ","FNCICLOMQ sin CICLOMQ");
				endBlock();
	...
	function readExpression2(){
		...
		if(readToken("operator")||readToken("minus")||readToken("xor")||(!noEquals&&readToken("="))||(!noTo&&readToken("LIMITE"))){
